...
const changeCurrentPassword = asyncHandler(async (req, res)=>{
    const {oldPassword, newPassword} = req.body //we can also add confirm password field
...
const updateAccountDetails = asyncHandler(async(req, res) =>{
    ...
    //tip : use seperate controllers for updating files to avoid network congestion
    if(!fullName || !email){
        throw new ApiError(400, "All fields are required")
    }
...
const updateUserCoverImage = asyncHandler(async(req, res)=>{
    const coverImageLocalPath = req.file?._path //got this through multer middleware
